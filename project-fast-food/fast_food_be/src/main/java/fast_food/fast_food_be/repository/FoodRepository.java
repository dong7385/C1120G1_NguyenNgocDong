...
//            "where food.flag = 0 and (concat(food.description_food,food.food_name) like %?1%)",nativeQuery = true)
//    List<Food>getListFood(String keySearch);

    @Query(value="select * " +
            "from food " +
            "left join  status_food on food.status_food_id = status_food.status_food_id " +
            "left join type_food on food.type_food_id = type_food.type_food_id " +
            "where food.flag = 0 and food.food_name = ?1 ",nativeQuery = true)
